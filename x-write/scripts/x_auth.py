#!/usr/bin/env python3
"""
X OAuth2 Authentication Helper
Automatically starts local server and handles callback
"""
...
def authenticate(timeout=120):
    ...
    print(f"🔐 Starting OAuth2 authentication...")
...
def main():
    ...
    if len(sys.argv) > 1 and sys.argv[1] == '--help':
        print("X OAuth2 Authentication Helper")
        print()
        print("Usage:")
        print("  python x_auth.py           # Start authentication flow")
        print("  python x_auth.py --status  # Check current token status")
        print()
        print("Requirements:")
        print(f"  - {CLIENT_CREDENTIALS_FILE}")
        print(f"  - Callback URI must be registered: {REDIRECT_URI}")
        sys.exit(0)
