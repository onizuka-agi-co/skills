...
- **ブックマーク監視**: 自分のブックマークを監視し、新規追加を検知
...
ブックマークの新規追加を検知して通知する機能。

### 必要なスコープ

- `bookmark.read` - ブックマーク読み取り権限

トークンにこのスコープがない場合は、X Developer Portalで権限を追加して再認証が必要。

### 使い方

```bash
# ブックマーク取得テスト
uv run skills/x-stream/scripts/x_bookmark_monitor.py test

# 新規ブックマーク確認
uv run skills/x-stream/scripts/x_bookmark_monitor.py check

# 状態確認
uv run skills/x-stream/scripts/x_bookmark_monitor.py status

# 状態クリア
uv run skills/x-stream/scripts/x_bookmark_monitor.py clear
```

### Discord通知

`data/x/x-discord-webhook.json` にWebhook URLを設定すると、新規ブックマークをDiscordに通知。
...
```bash
# 5分ごとにチェック
*/5 * * * * cd /config/.openclaw/workspace && uv run skills/x-stream/scripts/x_bookmark_monitor.py check
```
